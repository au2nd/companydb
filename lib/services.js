...
const collectCompanyDetails = async (context, companyList) => {
  ...
  const detailPromises = companyList.map((searchedCompany) =>
    limit(() =>
      getCompanyDetail(context, searchedCompany.link)
        .then((detail) => {
          printCompanyDetails(detail)
          return detail
        })
        .catch((error) => {
          console.error(`${searchedCompany.name}:`, error)
          return null
        })
    )
  )
  ...
  const collectedDetails = settledResults
